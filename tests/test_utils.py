...
import torch
from irtorch.utils import gauss_hermite, get_item_categories, split_data, impute_missing, _imv_newton_raphson, imv
...
def test__imv_newton_raphson():
    ...
    w = _imv_newton_raphson(negative_entropy)
...
def test_imv():
    ...
    data = torch.tensor([])
    # Call imv function
    imv_value = imv(model1, model2, data)
    # Compute expected imv
    log_geometric_means = torch.tensor([-0.5, -0.2])
    w = _imv_newton_raphson(log_geometric_means)
    expected_imv = (w[1] - w[0]) / w[0]
    # Check that imv_value matches expected_imv
    assert torch.allclose(imv_value, expected_imv, atol=1e-6), "The computed IMV does not match the expected value."
