...
def test_init_with_data(sample_data):
    model = SurprisalSpline(data=sample_data)
...
def test_forward():
    model = SurprisalSpline(latent_variables=2, item_categories=[2, 3])
    theta = torch.randn((3, 2), requires_grad=True)
    output = model(theta)
...
def test_log_likelihood():
    model = SurprisalSpline(item_categories=[2, 3])
...
def test_monotonicity():
    model = SurprisalSpline(item_categories=[2])
...
def test_with_missing_data(sample_data):
    ...
    model = SurprisalSpline(data=data_with_missing)
