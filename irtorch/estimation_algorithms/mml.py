...
from irtorch.models import BaseIRTModel, Spline, NestedLogit, Spline
...
class MML(BaseIRTAlgorithm):
    ...
    def _training_loop(
        ...
        if isinstance(model, Spline) or isinstance(model, Spline) or (isinstance(model, NestedLogit) and model.incorrect_response_model == "bspline"):
