...
@pytest.mark.parametrize("theta_estimation", ["ML", "NN"])
@pytest.mark.parametrize("guessing_probabilities", [None, [0.25, 0.5]])
def test_bit_score_starting_theta(bit_scales: BitScales, theta_estimation, guessing_probabilities, latent_variables):
    items = [0, 1]  
    ...
    with patch.object(bit_scales.model, 'latent_scores', mock_latent_scores):
        with patch.object(bit_scales.model, 'item_theta_relationship_directions', return_value=torch.ones(len(items), latent_variables)):
            with patch('irtorch._internal_utils.linear_regression', return_value=(None, torch.ones(latent_variables))):
                with patch('irtorch._internal_utils.random_guessing_data', return_value=torch.randn(10000, len(items))):
                    ...
                    if theta_estimation == "ML":
                        mock_latent_scores.assert_called()
