...
def residuals(
    ...
    torch.Tensor
        The residuals.
    """
    data, z = _evaluate_data_z_input(model, data, z, z_estimation_method)

    # 3D tensor with dimensions (respondents, items, item categories)
    if model.mc_correct is not None:
        probabilities = model.item_probabilities(z)
        ...
        residuals = data - model.expected_scores(z, return_item_scores=True)
...
def group_fit_residuals(
    ...
    for item, categories in enumerate(model.item_categories):
        # set non-existing categories to nan
        categories = categories + 1 if model.model_missing else categories
...
def infit_outfit(
    ...
    expected_scores = model.expected_scores(z, return_item_scores=True)
    probabilities = model.item_probabilities(z)
    observed_scores = data
    if model.mc_correct is not None:
        score_indices = torch.zeros(probabilities.shape[1], probabilities.shape[2])
        score_indices.scatter_(1, (torch.tensor(model.mc_correct) - 1 + model.model_missing).unsqueeze(1), 1)
        ...
        possible_scores[:, :, 1] = 1
        observed_scores = (data == torch.tensor(model.mc_correct) - 1).int()
    else:
...
def latent_group_probabilities(
    ...
    grouped_data_probabilties = _grouped_data_probabilities(model, grouped_data)
    grouped_model_probabilties = _grouped_z_probabilities(model, grouped_z)
    return grouped_data_probabilties, grouped_model_probabilties, group_mid_points

@torch.inference_mode()
def _grouped_z_probabilities(model: BaseIRTModel, grouped_z: tuple[torch.Tensor, ...]):
    """
    ...
    """
    group_probabilities = torch.zeros(len(grouped_z), len(model.modeled_item_responses), max(model.modeled_item_responses))
    for group_i, group in enumerate(grouped_z):
        item_probabilities = model.item_probabilities(group)
        group_probabilities[group_i, :, :] = item_probabilities.mean(dim=0)
    return group_probabilities

@torch.inference_mode()
def _grouped_data_probabilities(model: BaseIRTModel, grouped_data: tuple[torch.Tensor, ...]):
    """
    Computes the average probabilities for each potential item response for each group.
    ...
    torch.Tensor
        A 3D torch tensor with group averages. The first dimension represents the groups, the second dimension represents the items and the third dimension represents the item categories.
    """
    modeled_item_responses = model.modeled_item_responses
...
def sum_score_probabilities(
    ...
    """
    _validate_latent_density_method(model, latent_density_method)
    z_scores, weights = _get_z_scores_and_weights(
        model, latent_density_method, population_data, trapezoidal_segments, sample_size
    )
    probabilities = model.item_probabilities(z_scores)
    ...
    if model.mc_correct is not None:
        probabilities = sum_incorrect_probabilities(
            ...
            missing_modeled=model.model_missing
        )
        item_categories = [2] * len(model.modeled_item_responses)
    else:
    ...
    sum_score_probs = conditional_total_score_probs * weights.view(-1, 1)
    return sum_score_probs.sum(dim=0)

def _validate_latent_density_method(model: BaseIRTModel, latent_density_method: str) -> None:
...
def _get_z_scores_and_weights(
    ...
    if population_data is None:
        if isinstance(model.algorithm, (AE, VAE)):
            z_scores = model.algorithm.training_z_scores
        ...
        z_scores = model.latent_scores(population_data, z_estimation_method="NN", ml_map_device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)
    ...
    if latent_density_method in ["data", "encoder sampling"]:
        ...
        if latent_density_method == "encoder sampling":
            z_scores = model.algorithm.sample_latent_variables(
        ...
        (
            z_scores,
            weights,
        ) = _get_z_scores_and_weights_for_latent_density_methods(
...
def _get_z_scores_and_weights_for_latent_density_methods(
    ...
    weights = latent_density.pdf(z_scores)
...
def _evaluate_data_z_input(
    ...
    torch.Tensor, torch.Tensor
        The input data without missing values and the latent variable z scores.
    """
    if data is None:
        data = model.algorithm.train_data
    else:
        data = data.contiguous()

    if not model.model_missing:
        data = impute_missing(data, model.mc_correct, model.item_categories)

    if z is None:
        z = model.latent_scores(data=data, scale="z", z_estimation_method=z_estimation_method, ml_map_device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)

    data = fix_missing_values(data, model.model_missing, imputation_method="zero")
...
def approximate_latent_density(
    ...
    if approximation == "gmm":
        ...
        latent_density = _cv_gaussian_mixture_model(z_scores, cv_n_components)
    elif approximation == "qmvn":
        latent_density = QuantileMVNormal()
        latent_density.fit(z_scores)
    else:
        raise ValueError("Invalid approximation method. Choose either 'qmvn' or 'gmm'.")
    
    return latent_density

def _cv_gaussian_mixture_model(data: torch.Tensor, cv_n_components: list[int]) -> GaussianMixtureModel:
...
def min_max_z_for_integration(
    ...
    if z is None:
        if isinstance(model.algorithm, (AE, VAE)):
            z = model.algorithm.training_z_scores
            z_min = z.min(dim=0)[0]
            z_max = z.max(dim=0)[0]
            z_stds = z.std(dim=0)
        elif isinstance(model.algorithm, MML):
            z_min = torch.full((model.latent_variables,), -3)
            z_max = torch.full((model.latent_variables,), 3)
            z_stds = torch.ones(model.latent_variables)
    else:
        z_min = z.min(dim=0)[0]
        z_max = z.max(dim=0)[0]
        z_stds = z.std(dim=0)
