...
class BitScales:
    ...
    def bit_score_starting_z(
        ...
        if not start_all_incorrect:
            if train_z is None:
                if isinstance(self.model.algorithm, (AE, VAE)):
                    train_z = self.model.algorithm.training_z_scores
        ...
        if guessing_probabilities is None:
            if start_all_incorrect:
                ...
                starting_z = self.model.latent_scores(torch.zeros((1, len(items))).float(), z_estimation_method=z_estimation_method, ml_map_device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)
            else:
                ...
                for z in range(self.model.latent_variables):
                    ...
                    starting_z[:, z] = self.model.latent_scores(min_sum_score.float()[:, z], z_estimation_method=z_estimation_method, ml_map_device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)[:, z]
        else:
            ...
            if start_all_incorrect:
                ...
                guessing_z = self.model.latent_scores(random_data, z_estimation_method=z_estimation_method, ml_map_device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)
            else:
                ...
                for z in range(self.model.latent_variables):
                    ...
                    guessing_z[:, z] = self.model.latent_scores(random_data_z, z_estimation_method=z_estimation_method, ml_map_device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)[:, z]
    ...
    def bit_scores_from_z(
        ...
        if population_z is None:
            if z_estimation_method != "NN":
                ...
                population_z = self.model.latent_scores(self.model.algorithm.train_data, z_estimation_method=z_estimation_method, ml_map_device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)
            else:
                if isinstance(self.model.algorithm, (AE, VAE)):
                    logger.info("Using traning data z scores as population z scores for bit score computation.")
                    population_z = self.model.algorithm.training_z_scores
                elif isinstance(self.model.algorithm, MML):
    ...
    def bit_score_gradients(
        ...
        if hasattr(self.model.algorithm, "training_z_scores") and self.model.algorithm.training_z_scores is not None:
        # if isinstance(self.algorithm, (AE, VAE)):
            q1_q3 = torch.quantile(self.model.algorithm.training_z_scores, torch.tensor([0.25, 0.75]), dim=0)
        else:
    ...
    def bit_information(self, z: torch.Tensor, item: bool = True, degrees: list[int] = None, **kwargs) -> torch.Tensor:
        r"""
